...
#        for path in paths:
#            tts += len(path['obs'])
#        print('Total time steps so far:',tts)
...
tts = args.num_train_iter*args.episode_length*args.n_episode

training_log_all_experiment = np.array(return_log_all)
save_path = os.path.join('logs',args.exp_name+'_'+str(tts))
np.savetxt(save_path,training_log_all_experiment)
'''
Total time steps for PPO: num_train_iter*episode_length*n_episode
'''
